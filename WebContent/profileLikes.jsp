...
<link href="Stylesheet/likes.css" type="text/css" rel="stylesheet">
...
<body>
	...
	<div class="likesContainer">
		<%
			ArrayList<String> listReceived = UserProfile.fetchDataAsLikee();
			if (listReceived.size() <= 0) {
		%>
		<div class="likeNoData">No Data Found</div>
		<%
			} else {
				for (int i = 0; i < listReceived.size(); i += 2) {
		%>
		<div class="likesRow">
			<div class="likeeId">
				<a href="profileInfo_Cus_ViewOnly.jsp?pid=<%=listReceived.get(i)%>"><i class="fa fa-user-circle-o"></i>
				<%
					out.println("  " + listReceived.get(i));
				%></a>
			</div>
			<!-- End of likeeId -->
			<div class="likeTime">
				<%
					out.println("  " + listReceived.get(i + 1));
				%>
			</div>
			<!-- End of likeTime -->
		</div>
